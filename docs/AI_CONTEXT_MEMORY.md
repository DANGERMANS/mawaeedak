...
- Added bottom navigation in this fixed order:
  ...
  4. `التقويم`
  5. `المزيد`
- Added initial non-empty screens for Home, Salaries, Services, Calendar, and More.
...
Completed/created/updated on the migration branch:
...
Accept as partial only if it created a real mobile shell, did not modify forbidden areas, and documents that verification is missing.

Reject as final closure if typecheck was not run, Expo was not run, runtime was not verified, it claims done/ready without evidence, or it starts new phases before verification.
...
- Mobile bottom navigation order is fixed: `الرئيسية - الرواتب - الخدمات - التقويم - المزيد`.
- Services order is fixed: `احسب هدفك`, `حساب التكاليف`, `ذكرني`, `السفر`, `الدراسة والإجازات`, `الوظائف والأخبار`, `بطاقة اليوم`, `صوتك مسموع`.
